...
class igfuncController extends Controller
{
    public function trim_replace($data)
    {
        $a = trim($data);
        $a = str_replace('<', '', $a);
        $a = str_replace('>', '', $a);
    ...
    public function paginasi($reload, $page, $tpages)
    {
        $adjacents = 2;
    ...
    public static function proccess($ighost, $useragent, $url, $cookie = 0, $data = 0, $httpheader = array(), $proxy = 0, $userpwd = 0, $is_socks5 = 0)
    {
    ...
    public static function proccess_v2($ighost, $useragent, $url, $cookie = 0, $data = 0, $httpheader = array(), $proxy = 0, $userpwd = 0, $is_socks5 = 0)
    {
    ...
    public static function request($ighost, $useragent, $url, $cookie = 0, $data = 0, $httpheader = array(), $proxy = 0, $userpwd = 0, $is_socks5 = 0)
    {
        $url = $ighost ? 'https://i.instagram.com/api/v1/' . $url : $url;
        $ch  = curl_init($url);
        curl_setopt($ch, CURLOPT_USERAGENT, $useragent);
    ...
    public static function generate_useragent($sign_version = '107.0.0.27.121')
    {
        $resolusi = array(
            '1080x1776',
            '1080x1920',
    ...
    public function ava_hook($bound, $csrf, $file_url)
    {
        $eol  = "\r\n";
        $body = '';
        $body .= '--' . $bound . $eol;
    ...
    public static function generate_guid($tipe = 0)
    {
        $guid = sprintf('%04x%04x-%04x-%04x-%04x-%04x%04x%04x', mt_rand(0, 65535), mt_rand(0, 65535), mt_rand(0, 65535), mt_rand(16384, 20479), mt_rand(32768, 49151), mt_rand(0, 65535), mt_rand(0, 65535), mt_rand(0, 65535));
        return $tipe ? $guid : str_replace('-', '', $guid);
    }

    public function replace_friendship($teks, $tipe, $useragent, $cookies)
    {
    ...
    public function replace_media($teks, $tipe, $useragent, $cookies)
    {
        preg_match('#_from_(.*?)_limit_#', $teks, $from);
